...
class DataUnit(object):
    ...
    def next_batch(self, batch_size,trainset):
        """
        生成一批训练数据
        :param batch_size: 每一批数据的样本数 trainset：训练集
        :return: 经过了填充处理的QA对
        """
        data_batch = random.sample(trainset, batch_size)
        batch = []
        ...
        return batch
    def next_batch_test(self, batch_size,testset_q,testset_r,i):
        """
                生成一批测试/验证数据
                :param batch_size: 每一批数据的样本数 testset_q：验证/测试集问句 testset_r：验证/测试集问句回复 i:随机种子
                :return: 经过了填充处理的QA对
                """
        random.seed(i)
        data_batch = random.sample(testset_q, batch_size)
        random.seed(i)
        r_batch = random.sample(testset_r, batch_size)
        # print(data_batch[0:5])
        encoded = []
        length = []
        for q in data_batch:
            encoded_q = self.transform_sentence(q)
            q_len = len(encoded_q)
            length.append(q_len)
            encoded_q = encoded_q + [self.func_word2index(self.PAD)] * (self.max_q_len - q_len)
            encoded.append(encoded_q)
        # print("question:{}".format(data_batch))
        # print("r_batch:{}".format(r_batch))
        # print("encoded:{}".format(encoded))
        return encoded,length,r_batch
